...
class GameScreen(private val mGame: JumpActionGame) : ScreenAdapter() {
    ...
    // 効果音
    private var mClearSound: Sound
    private var mStepSound: Sound
    private var mStarSound: Sound
    private var mMissSound: Sound
    ...
    init {
        ...
        mScore = 0
        mHighScore = 0
        mClearSound = Gdx.audio.newSound(Gdx.files.internal("data/clear.mp3"))
        mStepSound = Gdx.audio.newSound(Gdx.files.internal("data/step.mp3"))
        mStarSound = Gdx.audio.newSound(Gdx.files.internal("data/star.mp3"))
        mMissSound = Gdx.audio.newSound(Gdx.files.internal("data/miss.mp3"))

        // ハイスコアをPreferencesから取得する
        mPrefs = Gdx.app.getPreferences("jp.techacademy.takayuki.ochiai.jumpactiongame")
    ...
    override fun render(delta: Float) {
        ...
        // 画面のクリア(塗り直し) おそらくこれをやらないと、前の描画が残り残像のようになる?
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (mRandom.nextFloat() > 0.6f) {
                ...
                if (mRandom.nextFloat() > 0.9f) {
                    texture = tripleStarTexture
                    isTriple = true
                } else {
                    texture = starTexture
                }
                val star = Star(texture, 0, 0, 72, 72, isTriple)

                star.setPosition(step.x + mRandom.nextFloat(), step.y + Star.STAR_HEIGHT + mRandom.nextFloat() * 3)
                mStars.add(star)
            }

            // Enemyを配置(ステージの後半から出現)
            if (mRandom.nextFloat() > 0.7f && y > WORLD_HEIGHT / 2) {
                val enemy = Enemy(enemyTexture, 0, 0, 120, 74)
                enemy.setPosition(step.x + mRandom.nextFloat() + 1, step.y + Star.STAR_HEIGHT + mRandom.nextFloat() * 3)
                mEnemy.add(enemy)
            }
    ...
    // 効果音を鳴らす
    private fun doSound(type: SOUND_TYPE) {
        when (type) {
            SOUND_TYPE.CLEAR -> mClearSound.play(1.0f)
            SOUND_TYPE.STEP -> mStepSound.play(1.0f)
            SOUND_TYPE.STAR -> mStarSound.play(1.0f)
            SOUND_TYPE.MISS -> mMissSound.play(1.0f)
        }
    }
}